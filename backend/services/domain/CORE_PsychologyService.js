...
class PsychologyService extends AbstractService {
     ...
     */
    async onInitialize() {
        this.logger.info('⚙️  Initializing psychology service...');
        ...
        this.refreshLLMService = async () => {
            this.logger.info('🔄 PsychologyService refreshing LLM service references...');
            this.structuredResponse = this.dependencies.enhancedStructuredResponse || this.dependencies.structuredResponse;
            this.jsonOrchestrator = this.dependencies.jsonProcessingOrchestrator;
        ...
        this.logger.info('✅ Psychology service initialized');
    }
...
Create a comprehensive psychological framework that captures the character's core traits, emotional patterns, and behavioral tendencies.`;
            ...
            try {
                ...
                this.logger.info(`🧠 Created psychological framework for ${personalityName}`);
                return framework;
                
            } catch (error) {
                this.logger.error('Failed to analyze personality framework:', error);
    ...
    async storePersonalityFramework(personalityId, framework) {
        return await this.withMetrics(async () => {
            ...
            this.logger.info(`✅ Stored psychology framework for personality: ${personalityId}`);
    ...
    async getPersonalityFramework(personalityId) {
        return await this.withMetrics(async () => {
            try {
                ...
                this.logger.error('Error getting personality framework via DAL:', error);
                return null;
            }
    ...
    async getCharacterState(sessionId) {
        return await this.withMetrics(async () => {
            try {
                ...
                this.logger.error('Error getting character state:', error);
                throw error;
            }
        }, 'getCharacterState');
    }
    ...
    async cleanupStateJsonFields(state) {
        ...
        if (state.current_motivations && typeof state.current_motivations === 'string') {
            this.logger.info(`🔧 Fixing malformed current_motivations: "${state.current_motivations}"`);
        ...
        if (state.active_interests && typeof state.active_interests === 'string') {
            this.logger.info(`🔧 Fixing malformed active_interests: "${state.active_interests}"`);
...
Convert to proper JSON format.`;
            ...
            this.logger.warn(`Failed to fix malformed JSON field "${malformedValue}":`, error.message);
            return fallback;
        }
    }
...
How would this naturally affect their internal state?`;
            ...
            try {
                ...
                this.logger.warn('🔄 Structured response failed (likely during model switch), using fallback:', structuredResponseError.message);
    ...
    async updateCharacterState(sessionId, updates) {
        ...
        try {
            ...
            this.logger.error('Error updating character state:', error);
            throw error;
        }
    }
...
- Contextual importance`;
            ...
            try {
                ...
                try {
                    ...
                    this.logger.warn('🔄 Memory weighting failed (likely during model switch), using fallback weights');
    ...
    async getPsychologyFramework(personalityId) {
        try {
            ...
            this.logger.warn('Failed to get psychology framework:', error.message);
            return null;
        }
    }
    ...
    async ensurePersonalityFramework(personality) {
        ...
        if (!framework) {
            this.logger.info(`🧠 Creating psychological framework for personality: ${personality.name}`);
    ...
    resetAnalytics() {
        ...
        this.logger.info('📊 Psychology service analytics reset');
    }
