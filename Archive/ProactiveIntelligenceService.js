...
class ProactiveIntelligenceService extends AbstractService {
    ...
    async onInitialize() {
        try {
            ...
            if (!this.logger) {
                console.warn(`No logger service available for ProactiveIntelligenceService, using console fallback`);
                this.logger = {
                    debug: (msg, ctx, meta) => console.debug(`[${ctx || 'ProactiveIntelligence'}] ${msg}`),
                    info: (msg, ctx, meta) => console.log(`[${ctx || 'ProactiveIntelligence'}] ${msg}`),
                    warn: (msg, ctx, meta) => console.warn(`[${ctx || 'ProactiveIntelligence'}] ${msg}`),
                    error: (msg, ctx, meta) => console.error(`[${ctx || 'ProactiveIntelligence'}] ${msg}`, meta?.error || '')
                };
            }
