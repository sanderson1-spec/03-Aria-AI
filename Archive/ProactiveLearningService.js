...
const { DateTimeUtils } = require('../utilities/datetime_utils');
...
class ProactiveLearningService extends AbstractService {
    ...
    async onInitialize() {
        try {
            ...
            if (!this.logger) {
                console.warn(`No logger service available for ProactiveLearningService, using console fallback`);
                this.logger = {
                    debug: (msg, ctx, meta) => console.debug(`[${ctx || 'ProactiveLearning'}] ${msg}`),
                    info: (msg, ctx, meta) => console.log(`[${ctx || 'ProactiveLearning'}] ${msg}`),
                    warn: (msg, ctx, meta) => console.warn(`[${ctx || 'ProactiveLearning'}] ${msg}`),
                    error: (msg, ctx, meta) => console.error(`[${ctx || 'ProactiveLearning'}] ${msg}`, meta?.error || '')
                };
            }
